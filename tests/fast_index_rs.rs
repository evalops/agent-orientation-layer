...
fn bare_path_like_queries_use_filter_only_fast_paths() {
    ...
    let hosted_column_location_fallback = search_repo_fast_filtered(
        repo.path(),
        "https://github.com/evalops/orient-search/blob/main/src/lib.rs#L40C9-L41C1",
        10,
        &filters,
    )
    .unwrap();
    assert_eq!(
        result_paths(&hosted_column_location_fallback),
        vec!["src/lib.rs"]
    );
    ...
    let raw_github_location_fallback = search_repo_fast_filtered(
        repo.path(),
        "https://raw.githubusercontent.com/evalops/orient-search/main/src/lib.rs",
        10,
        &filters,
    )
    .unwrap();
    assert_eq!(
        result_paths(&raw_github_location_fallback),
        vec!["src/lib.rs"]
    );
    let raw_github_slashy_branch_location_fallback = search_repo_fast_filtered(
        repo.path(),
        "https://raw.githubusercontent.com/evalops/orient-search/feature/search/src/lib.rs#L40-L45",
        10,
        &filters,
    )
    .unwrap();
    assert_eq!(
        result_paths(&raw_github_slashy_branch_location_fallback),
        vec!["src/lib.rs"]
    );
    assert_eq!(
    ...
    let gitlab_raw_location_fallback = search_repo_fast_filtered(
        repo.path(),
        "https://gitlab.com/evalops/orient-search/-/raw/main/src/lib.rs#L40-45",
        10,
        &filters,
    )
    .unwrap();
    assert_eq!(
        result_paths(&gitlab_raw_location_fallback),
        vec!["src/lib.rs"]
    );
    ...
    let github_actions_location_fallback = search_repo_fast_filtered(
        repo.path(),
        "::error file=src/lib.rs,line=40,col=9::target_entrypoint failed",
        10,
        &filters,
    )
    .unwrap();
    assert_eq!(
        result_paths(&github_actions_location_fallback),
        vec!["src/lib.rs"]
    );
    ...
    let hosted_column_location_indexed = index
        .search_filtered(
            "https://github.com/evalops/orient-search/blob/main/src/lib.rs#L40C9-L41C1",
            10,
            &filters,
        ...
        .unwrap();
    assert_eq!(
        result_paths(&hosted_column_location_indexed),
        vec!["src/lib.rs"]
    );
    ...
    let raw_github_location_indexed = index
        .search_filtered(
            "https://raw.githubusercontent.com/evalops/orient-search/main/src/lib.rs",
            10,
            &filters,
        ...
        .unwrap();
    assert_eq!(
        result_paths(&raw_github_location_indexed),
        vec!["src/lib.rs"]
    );
    let raw_github_slashy_branch_location_indexed = index
        .search_filtered(
            "https://raw.githubusercontent.com/evalops/orient-search/feature/search/src/lib.rs#L40-L45",
            10,
            &filters,
        ...
        .unwrap();
    assert_eq!(
        result_paths(&raw_github_slashy_branch_location_indexed),
        vec!["src/lib.rs"]
    );
    assert_eq!(
    ...
    let gitlab_raw_location_indexed = index
        .search_filtered(
            "https://gitlab.com/evalops/orient-search/-/raw/main/src/lib.rs#L40-45",
            10,
            &filters,
        ...
        .unwrap();
    assert_eq!(
        result_paths(&gitlab_raw_location_indexed),
        vec!["src/lib.rs"]
    );
    ...
    let github_actions_location_indexed = index
        .search_filtered(
            "::warning file=src/lib.rs,line=40,endLine=45::target_entrypoint failed",
            10,
            &filters,
        )
        .unwrap();
    assert_eq!(
        result_paths(&github_actions_location_indexed),
        vec!["src/lib.rs"]
    );
