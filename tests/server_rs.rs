...
fn runtime_search_auto_uses_live_repo_and_single_warmed_index() {
    ...
    assert_eq!(
        auto_retry["summary"]["primary_retry_status"],
        serde_json::json!("matched")
    );
    assert_eq!(
        auto_retry["summary"]["primary_retry_result_count"],
        serde_json::json!(1)
    );
    assert_eq!(
        auto_retry["summary"]["primary_retry_top_paths"],
        serde_json::json!(["src/auth.rs"])
    );
    assert_eq!(
    ...
    assert_eq!(
        auto_retry_indexed["summary"]["primary_retry_status"],
        serde_json::json!("matched")
    );
    assert_eq!(
        auto_retry_indexed["summary"]["primary_retry_result_count"],
        serde_json::json!(1)
    );
    assert_eq!(
        auto_retry_indexed["summary"]["primary_retry_top_paths"],
        serde_json::json!(["src/auth.rs"])
    );
    assert_eq!(
...
fn runtime_read_alias_accepts_live_index_and_shard_targets() {
    ...
    let live_gitlab_range = runtime.dispatch(ToolRequest {
        id: serde_json::json!("live-gitlab-range"),
        tool: "read_range".to_string(),
        arguments: serde_json::json!({
            "repo": repo.path(),
            "range": "https://gitlab.com/evalops/orient-search/-/blob/main/Cargo.toml#L1-2"
        }),
    });
    assert!(
        live_gitlab_range.error.is_none(),
        "{:?}",
        live_gitlab_range.error
    );
    let live_gitlab_range = live_gitlab_range.result.unwrap();
    assert_eq!(live_gitlab_range["path"], serde_json::json!("Cargo.toml"));
    assert_eq!(live_gitlab_range["start_line"], serde_json::json!(1));
    assert_eq!(live_gitlab_range["end_line"], serde_json::json!(2));
    assert_eq!(
        live_gitlab_range["summary"]["line_count"],
        serde_json::json!(2)
    );
...
fn runtime_search_plan_alias_accepts_live_index_and_shard_targets() {
    ...
    let live_advice = runtime.dispatch(ToolRequest {
        id: serde_json::json!("live-plan-advice"),
        tool: "search_plan".to_string(),
        arguments: serde_json::json!({
            "repo": repo.path(),
            "query": "issue definitely_missing",
    ...
    assert!(live_advice.error.is_none(), "{:?}", live_advice.error);
    let live_advice = live_advice.result.unwrap();
    assert_eq!(
        live_advice["primary_hint_kind"],
        serde_json::json!("drop_missing_terms")
    );
    assert!(
        live_advice["retry_cli"]
            .as_str()
            .unwrap()
            .contains("orient search")
    );
    assert!(
        live_advice["retry_jsonl"]
            .as_str()
            .unwrap()
    ...
    assert!(live_advice.get("primary_retry_request").is_none());
    assert!(live_advice.get("retry_requests").is_none());
    assert!(live_advice.get("planned_postings").is_none());
    ...
    let indexed_batch_advice = runtime.dispatch(ToolRequest {
        id: serde_json::json!("indexed-plan-batch-advice"),
        tool: "search_plan_batch".to_string(),
        arguments: serde_json::json!({
            "index": repo.path().join(".orient/index"),
            "queries": ["issue definitely_missing", "SessionManager definitely_missing"],
    ...
    assert!(
        indexed_batch_advice.error.is_none(),
        "{:?}",
        indexed_batch_advice.error
    );
    let indexed_batch_advice = indexed_batch_advice.result.unwrap();
    assert_eq!(
        indexed_batch_advice[0]["advice"]["primary_hint_kind"],
        serde_json::json!("drop_missing_terms")
    );
    assert!(
        indexed_batch_advice[0]["advice"]["retry_client_cli"]
            .as_str()
            .unwrap()
            .contains("orient client-jsonl")
    );
    assert!(indexed_batch_advice[0].get("summary").is_none());
    assert!(indexed_batch_advice[0].get("plan").is_none());
    ...
    let shard_advice = runtime.dispatch(ToolRequest {
        id: serde_json::json!("shard-plan-advice"),
        tool: "shard_query_plan".to_string(),
        arguments: serde_json::json!({
            "index_dir": repo.path().join(".orient-shards"),
            "query": "issue definitely_missing",
    ...
    assert!(shard_advice.error.is_none(), "{:?}", shard_advice.error);
    let shard_advice = shard_advice.result.unwrap();
    assert_eq!(
        shard_advice["primary_hint_kind"],
        serde_json::json!("drop_missing_terms")
    );
    assert_eq!(shard_advice["shard_count"], serde_json::json!(1));
    assert!(shard_advice.get("plans").is_none());
...
#[test]
fn runtime_short_file_filter_shard_search_uses_route_before_cold_load() {
    let workspace = tempfile::tempdir().unwrap();
    ...
    let manifest: serde_json::Value =
        serde_json::from_slice(&fs::read(shard_dir.path().join("manifest.json")).unwrap()).unwrap();
    let rust_index = manifest["shards"]
        .as_array()
        .unwrap()
        .iter()
        .find(|shard| shard["name"] == "rust-service")
        .unwrap()["index"]
        .as_str()
        .unwrap()
        .to_string();
    fs::remove_file(shard_dir.path().join("manifest.bin")).unwrap();
    fs::remove_file(shard_dir.path().join(rust_index)).unwrap();
    ...
    assert!(response.error.is_none(), "{:?}", response.error);
    let result = serde_json::to_string(&response.result).unwrap();
    assert!(result.contains("go-service/go.mod"), "{result}");
    assert!(!result.contains("rust-service"), "{result}");
    assert_eq!(runtime.cached_shard_manifest_count(), 0);
    assert_eq!(runtime.cached_index_count(), 1);
}
...
#[test]
fn tcp_daemon_bench_reports_concurrent_search_latency() {
    let binary = assert_cmd::cargo::cargo_bin("orient");
    let repo = tempfile::tempdir().unwrap();
    fs::create_dir_all(repo.path().join(".git")).unwrap();
    write(
    ...
    assert_eq!(report["mode"], serde_json::json!("daemon"));
    assert_eq!(report["concurrency"], serde_json::json!(2));
    assert_eq!(report["summary"]["query_count"], serde_json::json!(1));
    assert_eq!(report["summary"]["sample_count"], serde_json::json!(4));
    assert_eq!(
        report["queries"][0]["query"],
        serde_json::json!("issue token")
    );
    assert_eq!(report["queries"][0]["result_count"], serde_json::json!(1));
    assert!(report["queries"][0]["p95_ms"].as_f64().unwrap() >= 0.0);
}

#[test]
fn tcp_daemon_read_bench_reports_concurrent_range_latency() {
    let binary = assert_cmd::cargo::cargo_bin("orient");
    let repo = tempfile::tempdir().unwrap();
    fs::create_dir_all(repo.path().join(".git")).unwrap();
    write(
    ...
    let report: serde_json::Value = serde_json::from_slice(&output.stdout).unwrap();
    assert_eq!(report["mode"], serde_json::json!("daemon_read_range"));
    assert_eq!(report["concurrency"], serde_json::json!(2));
    assert_eq!(report["summary"]["query_count"], serde_json::json!(1));
    assert_eq!(report["summary"]["sample_count"], serde_json::json!(4));
    assert_eq!(
        report["queries"][0]["query"],
        serde_json::json!("src/auth.rs:1:1")
    );
    assert_eq!(report["queries"][0]["result_count"], serde_json::json!(1));
    assert!(report["queries"][0]["p95_ms"].as_f64().unwrap() >= 0.0);
}
...
#[test]
fn tcp_daemon_mix_bench_reports_concurrent_search_and_read_latency() {
    let binary = assert_cmd::cargo::cargo_bin("orient");
    let repo = tempfile::tempdir().unwrap();
    fs::create_dir_all(repo.path().join(".git")).unwrap();
    write(
    ...
    let report: serde_json::Value = serde_json::from_slice(&output.stdout).unwrap();
    assert_eq!(report["mode"], serde_json::json!("daemon_mix"));
    assert_eq!(report["concurrency"], serde_json::json!(4));
    assert_eq!(report["summary"]["query_count"], serde_json::json!(2));
    assert_eq!(report["summary"]["sample_count"], serde_json::json!(8));
...
fn tcp_daemon_churn_bench_reports_refresh_metrics_without_fallback() {
    ...
    let report: serde_json::Value = serde_json::from_slice(&output.stdout).unwrap();
    assert_eq!(report["mode"], serde_json::json!("daemon_churn"));
    assert_eq!(report["concurrency"], serde_json::json!(2));
    assert_eq!(report["summary"]["sample_count"], serde_json::json!(4));
    ...
    assert_eq!(
        report["queries"][0]["query"],
        serde_json::json!("search:orient_churn_token")
    );
    assert_eq!(report["queries"][0]["fallback_count"], serde_json::json!(0));
    assert!(
        !repo
            .path()
            .join(".orient-churn-bench")
...
#[test]
fn tcp_daemon_warms_only_requested_shard_repo_on_startup() {
    let workspace = tempfile::tempdir().unwrap();
    let auth_repo = workspace.path().join("auth");
    write(
        &auth_repo.join("src/lib.rs"),
