...
fn cli_outputs_tool_manifest() {
    ...
    cmd.args(["tool-manifest", "--format", "json"])
        ...
        .stdout(predicate::str::contains("register_shards"))
        .stdout(predicate::str::contains("warm_shards"))
        .stdout(predicate::str::contains("single_warmed_index"))
        .stdout(predicate::str::contains("single_registered_shard_dir"))
...
#[test]
fn cli_help_stays_focused_on_local_code_search() {
    let mut cmd = Command::cargo_bin("orient").unwrap();
    cmd.arg("--help")
        .assert()
        .success()
...
fn cli_search_auto_selects_live_indexed_and_shard_surfaces() {
    ...
    shard
        ...
        .stdout(predicate::str::contains("\"total_shards\":1"))
        .stdout(predicate::str::contains("\"selected_shards\":1"))
    ...
    assert_eq!(plan["retry_requests"][0]["arguments"]["query"], "lang:rust");
    assert_eq!(plan["retry_requests"][0]["arguments"]["language"], "rust");
    assert!(plan["retry_requests"][0]["arguments"]["file"].is_null());

    let mut diagnosed_path_typo = Command::cargo_bin("orient").unwrap();
    let output = diagnosed_path_typo
        .args([
            "search-auto",
            "--repo",
            repo.path().to_str().unwrap(),
    ...
    let value: serde_json::Value = serde_json::from_slice(&output.stdout).unwrap();
    let plan = &value["query_plan_result"];
    assert_eq!(plan["repair_hints"][0]["kind"], "replace_path_filter");
    assert_eq!(
        plan["retry_requests"][0]["arguments"]["query"],
        "path:src/auth.rs"
    );
...
fn cli_outputs_repo_map_and_reads_ranges() {
    ...
    repo_map
        ...
        .stdout(predicate::str::contains("\"status\":\"mapped\""))
        .stdout(predicate::str::contains("\"entrypoint_count\""))
    ...
    let mut hosted_column_link_read_range = Command::cargo_bin("orient").unwrap();
    hosted_column_link_read_range
        .args([
            "read-range",
            "--repo",
            repo.path().to_str().unwrap(),
            "https://github.com/evalops/orient-search/blob/main/src/auth.rs#L5C9-L6C1",
        ])
        .assert()
        .success()
        .stdout(predicate::str::contains("\"start_line\":5"))
    ...
    let mut gitlab_read_range = Command::cargo_bin("orient").unwrap();
    gitlab_read_range
        .args([
            "read-range",
            "--repo",
            repo.path().to_str().unwrap(),
            "https://gitlab.com/evalops/orient-search/-/blob/main/src/auth.rs#L5-6",
        ])
        .assert()
        .success()
        .stdout(predicate::str::contains("\"start_line\":5"))
    ...
    let mut rust_diagnostic_read_range = Command::cargo_bin("orient").unwrap();
    rust_diagnostic_read_range
        .args([
            "read-range",
            "--repo",
            repo.path().to_str().unwrap(),
            "--",
            "--> src/auth.rs:5:13: borrowed value does not live long enough",
        ])
        .assert()
        .success()
        .stdout(predicate::str::contains("\"path\":\"src/auth.rs\""))
    ...
    let mut rust_diagnostic_no_message_read_range = Command::cargo_bin("orient").unwrap();
    rust_diagnostic_no_message_read_range
        .args([
            "read-range",
            "--repo",
            repo.path().to_str().unwrap(),
    ...
    let mut rust_diagnostic_block_read_range = Command::cargo_bin("orient").unwrap();
    rust_diagnostic_block_read_range
        .args([
            "read-range",
            "--repo",
            repo.path().to_str().unwrap(),
    ...
    let go_panic_stack = format!(
        "panic: boom\n\nmain.issueToken()\n\t{}:5 +0x20",
        absolute_auth_path.display()
    );
    let mut go_panic_read_range = Command::cargo_bin("orient").unwrap();
    go_panic_read_range
        .args([
            "read-range",
            "--repo",
...
fn cli_reports_index_and_shard_freshness() {
    ...
    shard_status
        ...
        .stdout(predicate::str::contains(
            "\"manifest_route_substring_filter_shards\"",
        ))
...
fn cli_builds_and_searches_persistent_index() {
    ...
    assert!(
        index_symbol_batch[1]["summary"]["top_paths"]
            .as_array()
            .unwrap()
            .contains(&serde_json::json!("src/auth.rs")),
        "{index_symbol_batch:?}"
