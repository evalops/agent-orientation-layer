...
use orient::repo_index::{
    ...
    attach_result_related_symbol_requests, language_for, normalize_language_filter,
...
struct BenchSummary {
    ...
    #[serde(default, skip_serializing_if = "Option::is_none")]
    daemon_rss_start_bytes: Option<u64>,
    #[serde(default, skip_serializing_if = "Option::is_none")]
...
fn compact_query_plan_batch_result(query: String, plan: &QueryPlan) -> Value {
    let mut item = Map::new();
    item.insert("query".to_string(), Value::String(query));
    item.insert(
        "summary".to_string(),
        serde_json::to_value(plan.compact_summary()).unwrap_or(Value::Null),
    );
    if let Some(next_action) = &plan.next_action {
        item.insert(
            "next_action".to_string(),
            serde_json::to_value(next_action).unwrap_or(Value::Null),
        );
    }
    Value::Object(item)
}
...
fn daemon_rss_bytes_from_status(status: &Value) -> Option<u64> {
    status
        .get("footprint")
        .and_then(|footprint| footprint.get("process_rss_bytes"))
        .and_then(Value::as_u64)
}
