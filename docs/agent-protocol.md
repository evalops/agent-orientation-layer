...
- `search_auto_batch`, `search_batch`, `indexed_search_batch`, or `search_shards_batch` when an agent wants to try several query formulations in one round trip. Each explicit search batch item returns `summary` with `status`, `result_count`, grouped duplicate count when applicable, top paths, top directories, top extensions, top languages, and score bounds, target-aware `query_plan_request`, `repo_map_request`, read follow-ups, and `next_action`; hits point `next_action` at the batch read request, while empty items point it at the plan request. Shard-backed `search_auto_batch`, `search_batch` with `index_dir`, and explicit `search_shards_batch` summaries include `shard_route` so adapters can see whether a query fanned out broadly. `search_auto_batch` items also expose compact `query_plan_summary` when diagnostics are attached. On shard paths, `search_auto_batch` and explicit `search_shards_batch` apply `refresh_if_stale:true` once across the selected shard roots instead of refreshing per query. The JSON-lines `search_batch` tool accepts `repo`, `index`, or `index_dir` for the same target-aware plain result shape as `search`; the CLI mirrors this as `search-batch --repo`, `search-batch --index`, or `search-batch --index-dir`. The indexed and shard-specific batch tools remain available for explicit adapters.
- `search_plan`, `indexed_query_plan`, or `shard_query_plan` when a search returns empty or suspicious results and the agent needs missing terms plus retry hints. JSON-lines `search_plan` accepts `repo`, `index`, or `index_dir` for target-aware diagnostics; explicit `search_query_plan`, `indexed_query_plan`, and `shard_query_plan` remain available for adapters that prefer surface-specific tools. Plans include top-level `summary`, `next_action`, `primary_retry_request`, and ready-to-send `retry_requests` when a repair hint has a suggested query; compact summaries include up to four `top_repair_hints` so agents can act on facet or relaxation hints before opening the full plan. plan batch items include top-level `summary` and promote the same `next_action` at the item level. Pass JSON-lines `summary:true` or add `--summary` to plan CLI commands when an agent only needs the compact diagnosis, retry request, and next action. The CLI mirrors target-aware planning as `search-plan --repo`, `search-plan --index`, or `search-plan --index-dir`; explicit `index-plan` and `shard-plan` remain available.
...
CLI path flags accept the same agent-oriented aliases as query strings:
`--path`, `--dir`, `--folder`, `--in`, `--inside`, `--under`, and `--within`.
CLI searches can also be filter-only, for example `orient search --repo . --symbol parse_query`.

Query strings support filters such as `repo:service`, `branch:feature/auth`, `origin:example/service`, `path:src/auth` or `dir:src/auth` / `folder:src/auth` / `in:src/auth` / `under:packages/api` / `within:crates/search`, `file:auth.rs` or `filename:auth.rs`, `file:*.rs`, `path:src/*gateway.rs`, `path:src\auth.rs`, `line:42` or `target_line:42`, `lang:rust` or shorthand `lang:rs` / `lang:ts` / `lang:py` / `lang:cpp` / `lang:csharp` / `lang:shell` / `lang:makefile` / `lang:justfile`, `ext:rs`, `symbol:SessionManager`, `kind:function`, `type:function`, `kind:target` for Makefile/Justfile recipes, GitHub Actions jobs, and Bazel BUILD rule names, `kind:script` for package.json and pyproject scripts, `kind:package` for package.json package names, Cargo package names, pyproject package names, Go module paths, Maven coordinates, and Gradle project names, `kind:bin` / `kind:example` / `kind:bench` for Cargo manifest entries, `kind:service` for Docker Compose services, `kind:stage` for Dockerfile build stages, shorthand symbol-kind filters like `fn:issue_token`, `class:SessionManager`, `recipe:release`, `script:typecheck`, `package:auth-api`, `service:api`, `stage:builder`, or `bin:auth-worker`, `dep:react`, `import:crate::auth`, `test:false`, `is:test`, `is:source`, `code:true`, `code:false`, `is:code`, `is:docs`, `generated:false`, `is:generated`, positive content aliases like `content:"issue token"` or `text:gateway`, negative filters with `-` or `!` such as `-path:docs`, `!file:*test.rs`, `-folder:vendor`, `!under:vendor`, `!is:generated`, `-lang:md`, `-branch:wip`, `-origin:legacy`, `-kind:class`, `-dep:legacy`, or `-import:old_api`, negative alias prefixes such as `not:docs`, `exclude:generated`, and `without:path:vendor` / `without:within:fixtures`, bare surface exclusions like `!docs`, `!generated`, `!vendor`, `!node_modules`, and `!target`, bare negative content terms like `-deprecated`, `!deprecated`, or `exclude:deprecated`, and quoted phrases like `"issue token"`. Multi-token queries use AND behavior by default; use `mode:any` in the query or `any_terms:true` in JSON-lines calls for broad orientation searches. Indexed search plans `symbol:` and `kind:` filters through symbol postings and also treats identifier-shaped raw terms such as `SessionManager` and `agent_instructions` as symbol planning hints when a matching symbol exists, while ordinary spaced concept queries stay broad.
Bazel labels such as `//tools/search:orient_cli` and `:agent_smoke_test` infer `kind:target` plus the label's target name; package labels also add a path filter for the package portion. The same inference applies when a single label appears inside a Bazel command such as `bazel test //tools/search:orient_cli`.
...
Bare single-token filename and path-like queries such as `Cargo.toml`, `README.md`, or `src/lib.rs` are inferred as `file:` / `path:` filters so agents that type the file they want get the file, not references to its name. Pytest node IDs such as `tests/test_auth.py::test_login` are inferred as path filters too, including simple commands like `pytest tests/test_auth.py::test_login -q` or `python -m pytest tests/test_auth.py::test_login`. JS test commands such as `pnpm vitest src/auth.test.ts`, `npm test -- ./src/auth.test.ts`, and `npx jest auth.spec.tsx` infer `path:` / `file:` filters for obvious `.test.` / `.spec.` files and `__tests__` paths. Package script commands such as `npm run typecheck`, `yarn run build:prod`, and common shortcut forms like `pnpm lint` infer `kind:script symbol:<script>`. Task commands such as `make deploy` and `just release target=prod` infer `kind:target symbol:<target>`. Simple Rust test commands such as `cargo test parser_accepts_locations` and `cargo test query::tests::parser_accepts_locations` infer `kind:function symbol:parser_accepts_locations`; integration-test commands such as `cargo test --test parser_rs parser_accepts_locations` infer `path:tests/parser_rs.rs kind:function symbol:parser_accepts_locations`; simple Go test commands such as `go test ./pkg/auth -run TestLoginFlow` and `go test ./pkg/auth -run=TestLoginFlow` infer `path:pkg/auth kind:function symbol:TestLoginFlow`; Maven and Gradle selectors such as `mvn test -Dtest=GatewayTest#routesPayment` and `./gradlew test --tests com.example.GatewayTest.routesPayment` infer `file:GatewayTest.* kind:function symbol:routesPayment`. Use `content:Cargo.toml`, `text:README.md`, or `term:src/lib.rs` when the literal string is the target.
Bare pasted locations such as `src/lib.rs:42`, `src/lib.rs:42:9`,
`src/lib.rs#L42-L45`, `src/lib.rs#L42C9-L45C1`, copied lines such as
`src/lib.rs:42: pub fn issue_token`, Markdown-style file links, common hosted
code links, and stack-frame forms such as `at issueToken (src/lib.rs:42:9)`
...
`#Lstart-Lend` anchors. Azure DevOps and Visual Studio `_git` links are
normalized from `path=`, `line=`, and `lineEnd=` query parameters.
...
`test:true` / `is:test` matches common test layouts across languages, including `tests/`, `__tests__/`, `spec/`, `_test.go`, `_test.rs`, `.test.tsx`, and `.spec.ts`; `test:false` / `is:source` excludes those paths so agents can jump between production code and nearby tests without hand-written path filters. Bare positive `test` / `tests` stays a content term so ordinary discovery queries such as `parser test helper` do not accidentally scope themselves to test files; use `!test` / `!tests` as shorthand for excluding tests.

`code:true` / `is:code` matches implementation source-code languages such as Rust, Python, TypeScript, JavaScript, Go, Ruby, Java, Kotlin, Swift, C, C++, C#, and shell scripts while excluding docs/config formats like Markdown, TOML, JSON, YAML, and text; `code:false` / `is:docs` does the inverse for prose/config searches.
...
`filename` / `file_name`, `directory` / `folder`, normalized language shorthands such as `rs`, `ts`, `py`, `js`, `cpp`, `cs`, `sh`, `bash`, `make`, `just`, and `md`, and `exclude_*` aliases such as `exclude_folder`, `exclude_lang`, `exclude_ext`, `exclude_kind`,
...
`interface`/`interfaces`, `target`/`recipe`/`task` for Makefile targets,
Justfile targets, GitHub Actions jobs, and Bazel BUILD rule names, `script` / `package-script` for package.json and pyproject scripts,
`package` for package.json package names, Cargo package names, pyproject
package names, Go module paths, Maven coordinates, and Gradle project names,
`service` for Docker Compose services, `stage` for Dockerfile build stages, and
`bin` / `example` / `bench` for Cargo manifest entries.
...
returned canonical result ranges represent additional duplicate paths, and ask
batch read tools for summarized `{summary,ranges}` responses by default.
...
Read-range tools accept `/` or `\` separators in repo-relative paths and reject parent-directory escapes after separator normalization. Live repo reads also accept absolute copied paths only when they canonicalize inside the selected repository; outside paths and symlink escapes are rejected. Shard range and related-context tools accept exact shard-prefixed paths from search hits, such as `service/src/auth.rs`, and also accept unqualified paths like `src/auth.rs` when they resolve to exactly one shard. Ambiguous unqualified paths fail with a prompt to use `<repo>/<path>`.

`read_range` / `open_range` and `read_ranges` / `open_ranges` are target-aware convenience tools: pass `repo`, `index`, or `index_dir` to read from a live repository, persistent index, or shard directory with one adapter path. With no explicit target, protocol clients can pass `cwd` to resolve repository-relative paths inside the active checkout. The single-read `path` and batch `ranges` entries accept copied locations such as `path:line`, `path:start-end`, `path:line: copied text`, `path:start-end: copied text`, compiler diagnostics and pasted diagnostic blocks containing `--> path:line:column`, Python traceback frames such as `File "path", line N`, JavaScript stack frames such as `at fn (path:line:column)`, Go panic stack locations such as a function frame followed by `path:line +0x...`, `path#Lstart-Lend`, `path#LstartCcol-LendCcol`, Markdown links, and common hosted code links with fragment or query-string line anchors; `read_ranges.ranges` can be one compact string, one `{path,start,lines}` object, or an array mixing both. Single reads may also pass `range` as a compact string or object with the same shape as a search result's `read_range`. Range objects accept `start_line`, `end_line`, and `line_count` aliases, so adapters can feed search-result `line_range` shaped data back into read tools without translating it first. Batch reads dedupe identical entries and merge overlapping exact ranges before enforcing the total-line budget; symbol-scoped ranges stay separate because they resolve around nearby definitions. Batch reads are capped by range count and total requested lines, so large inspections should be split into smaller follow-up calls. Pass `include_summary:true` to `read_ranges`, `read_index_ranges`, or `read_shard_ranges` when an adapter wants `{summary,ranges}` with opened range count, total returned lines, unique path count, top paths, top directories, top extensions, and top languages; otherwise the response remains the bare range array. The explicit `read_index_range` and `read_shard_range` families remain available for wrappers that want surface-specific tools.
Bitbucket `#lines-start:end` anchors are normalized to the same bounded range
width as GitHub-style `#Lstart-Lend` anchors. GitLab `#Lstart-end` anchors are
accepted for both search and read-range calls. Azure DevOps `path=/...&line=...`
links are accepted for both search and reads.
Parenthesized compiler locations such as `path(line,column): error ...` are
accepted for both search and read-range calls.
JVM stack frames such as `at package.Class.method(File.java:line)` are accepted
too.

`related_files` and `related_symbols` are target-aware too: pass `repo`, `index`, or `index_dir` to get nearby files or definitions from the same target style as `search`, or pass `cwd` when using the shared daemon without an explicit target. The CLI mirrors this as `related --repo`, `related --index`, or `related --index-dir`, and likewise for `related-symbols`; pass `--format json` for explicit wrapper contracts and `--include-read-batch` for the wrapped `{summary, results, read_batch_request, next_action}` shape. Related-file and related-symbol tools accept the same structured scoping filters as search, with `path` reserved for the anchor file; use fields such as `test`, `generated`, `lang`, `file`, `exclude_path`, or `exclude_content` to control returned neighbors. Protocol clients can pass `include_read_batch:true` to receive `summary` with `status`, `result_count`, top paths, top directories, top extensions, top languages, and score bounds, plus `results`, `read_batch_request`, and `next_action` instead of the default result array. Related-symbol summaries also include compact `top_symbols` and `symbol_kinds` when definitions were found, so wrappers can choose between opening nearby functions, types, or tests without parsing every result first. For shard `related_symbols`, include the search-hit `path` so Orient can keep the lookup inside the right shard or alias scope. The explicit `related_index_*` and `related_shard_*` tools remain available.
...
orient open-index-ranges --index "$ORIENT_INDEX" --summary --range src/auth.rs:1:80
